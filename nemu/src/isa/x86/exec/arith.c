...
make_EHelper(cmp) {
  rtl_sub(&s0, &id_dest->val, &id_src->val);
  // Log("id_dest->val = 0x%x, id_src->val = 0x%x, s0 = 0x%x\n", id_dest->val, id_src->val, s0);
...
make_EHelper(inc) {
  ...
  // update OF
  rtl_li(&ir, 1);
  rtl_is_add_overflow(&s0, &s1, &id_dest->val, &ir, id_dest->width);
  rtl_set_OF(&s0);
...
make_EHelper(dec) {
  ...
  // update OF
  rtl_li(&ir, 1);
  rtl_is_sub_overflow(&s0, &s1, &id_dest->val, &ir, id_dest->width);
  rtl_set_OF(&s0);
